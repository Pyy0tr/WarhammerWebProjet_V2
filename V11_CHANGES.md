...
### Règle de détachement manquante (2026-07-02) — bug pré-existant, touchait V10 ET V11
Signalé par l'utilisateur : chaque détachement a normalement, en plus des Enhancements et Stratagems, une **"Rules"** — la règle passive du détachement (ex. Black Templars → Companions of Vehemence → "Righteous Fervour", reroll des Advance/Charge). Absente de la page Detachments dans les deux éditions.
- **Cause identifiée** : `fetch_gamedatacards.py`/`_v11.py` ne lisaient que `data.rules.army` (règles de faction, déjà extraites en `army_rules`) et ignoraient complètement `data.rules.detachment` — un array séparé, présent dans la source pour les deux éditions, qui contient exactement cette règle par détachement (même forme que `army_rules`, un niveau d'imbrication en plus car groupé par détachement). Confirmé en inspectant directement `10th/gdc/blacktemplar.json` et `11th/gdc/blacktemplar.json` — "Righteous Fervour" présent mot pour mot dans les deux.
- **Fix pipeline** : nouvelle fonction `detachment_rules()` dans les deux scripts (miroir de `army_rules()`, groupée par nom de détachement), champ `"rules"` ajouté à chaque détachement dans `det_list`. Les deux fichiers régénérés : `gdc.json` (202/202 détachements ont maintenant une règle) et `gdc_v11.json` (266/266). Toujours des scripts manuels (pas dans `sync_bsdata.yml`, inchangé).
- **Fix UI** : `DetachmentsPage.jsx` → nouvelle section "Detachment Rule" dans `DetachmentSection`, affichée en premier (avant Enhancements/Stratagems, ordre du livre de règles), badge "RULE" dans l'en-tête repliable, réutilise `<AbilityText>` comme les autres sections.
- **Clarification** : l'exemple "Purge the Foe" mentionné par l'utilisateur en même temps n'est PAS lié à ce bug — c'est une ability de datasheet (unité), pas une règle de détachement (vérifié : absent des données V11 actuelles côté GDC, présent côté BSData comme ability d'unité "Infernus Squad"). Aucune action nécessaire dessus.
- Testé en navigateur réel sur Black Templars/Companions of Vehemence (V10), section "Detachment Rule" affichée correctement avant Enhancements/Stratagems, 0 erreur console.

---

## 5. Impact UI / contenu

- ~~`keywords.js` : mettre à jour rule/note/tip pour HEAVY, PSYCHIC, CLEAVE~~ → **Fait** (`keywords_v11.js`, texte de règle confirmé pour les trois). HAZARDOUS reste non simulé, seuil 1-ou-2 toujours pas confirmé.
- ~~Toggle contexte `cover`~~ → **Fait (2026-07-01)** : `DefenderPanel.jsx` affiche un libellé edition-aware ("-1 to attacker's hit roll" en V11 vs "+1 to armour save" en V10). Le cas MONSTER/VEHICLE sans bénéfice de couverture reste non modélisé (question ouverte, section 6).
- ~~Toggle contexte "Attacker moved this turn" (Heavy)~~ → **Fait (2026-07-02)** : libellé V11 reflète maintenant les vraies conditions disqualifiantes ("Engaged, deployed this turn, or moved >3\" (loses Heavy +1)"). Le calcul reste identique (`attacker_moved` booléen inchangé) — seule la justification affichée change, comme prévu (voir commentaire `simulation_v11.js`).
- **Bug de copie corrigé (2026-07-02)** : le tooltip du sélecteur V10/V11 (`Navbar.jsx`) et un commentaire dans `dataStore.js` affirmaient encore que "le simulateur utilise toujours les règles V10" — obsolète depuis le fork du moteur (2026-07-01), corrigé pour refléter l'état réel (moteur V11 propre, données communautaires non recoupées avec le rulebook officiel).
- **Sélecteur V10/V11 restreint aux pages pertinentes (2026-07-02)** : le toggle était affiché sur toutes les pages, y compris Home, Learn, et les pages compte/auth qui n'ont aucun comportement dépendant de l'édition. Restreint aux 6 pages qui lisent réellement `edition` : Factions, Armies, Simulator, Keywords, Detachments, Combos (`Navbar.jsx`, `EDITION_AWARE_PATHS`).
- SEO/branding : mentions "V11 / 11th edition" toujours absentes de la page d'accueil et du SEO généré (`generate-pages.js`) — cohérent avec le fait que V11 reste un toggle preview et non une URL séparée ; décision à prendre si on veut en faire mention pour le marketing/SEO externe.
...
- BSData V11 officiel : **résolu 2026-07-08** — `vflam/wh40k-11e` a été adopté par l'org `BSData/wh40k-11e` (le fork perso de vflam n'existe plus sur GitHub, seul `vflam/wh40k-10e` reste). Pipeline repointé sur `BSData/wh40k-11e`, voir historique ci-dessous.
...
- 2026-07-02 : audit complet des pages/composants pour préparer un "passage propre" à V11 — Simulator/Factions/Armies/Keywords/Detachments/Combos confirmés edition-aware et testés, Home/Learn/Onboarding/Welcome/Feedback/Admin/Reset-Password confirmés hors périmètre (contenu générique, pas d'edition awareness nécessaire). Trois correctifs appliqués : (1) tooltip Navbar + commentaire `dataStore.js` obsolètes affirmant encore que le simulateur "utilise toujours les règles V10" (faux depuis le fork du moteur), (2) libellé du toggle contexte Heavy rendu edition-aware (alignement avec Cover/Plunging Fire), (3) sélecteur V10/V11 restreint aux 6 pages où il a un effet réel (masqué sur Home/Learn/auth — `EDITION_AWARE_PATHS` dans `Navbar.jsx`). Testé via Playwright-chromium (toggle absent/présent selon la route confirmé), pushé et déployé.
- 2026-07-02 : bug signalé par l'utilisateur — la "Rules" (règle passive) de chaque détachement (ex. Companions of Vehemence → "Righteous Fervour") manquait sur la page Detachments. Trouvé pré-existant sur V10 **et** V11 : les deux scripts ne lisaient que `data.rules.army`, jamais `data.rules.detachment` (array séparé, confirmé présent dans les deux sources GDC). Fix : nouvelle fonction `detachment_rules()` (miroir de `army_rules()`) dans les deux pipelines, nouvelle section "Detachment Rule" dans `DetachmentsPage.jsx` affichée avant Enhancements/Stratagems. `gdc.json`/`gdc_v11.json` régénérés : 202/202 et 266/266 détachements ont maintenant leur règle. Testé en navigateur réel sur Black Templars, 0 erreur console.
- 2026-07-08 : cron `sync_bsdata.yml` en échec depuis quelques jours (`404` sur `api.github.com/repos/vflam/wh40k-11e/commits/main`). Cause : le fork perso `vflam/wh40k-11e` a été adopté/migré vers l'org officielle `BSData/wh40k-11e` (confirmé — le repo `vflam/wh40k-11e` renvoie 404 même sur l'endpoint repo lui-même, et le compte `vflam` n'a plus que `wh40k-10e`). `BSData/wh40k-11e` est actif (commits le jour même), même structure de fichiers JSON par faction. Fix : `pipeline/fetch_bsdata_v11.py` repointé sur `BSData/wh40k-11e` (défaut + docstring), commentaire mis à jour dans `sync_bsdata.yml`.
- 2026-07-09 : incident GitHub Actions ("Delays starting Actions runs", runners hébergés) ayant fait échouer un run CI/CD par manque de runner disponible après ~15 min de file d'attente — sans lien avec le code. À cette occasion, fréquence du cron `sync_bsdata.yml` réduite de 12h à 48h (`0 */12 * * *` → `0 0 */2 * *`) pour diviser par 4 le nombre de déclenchements de workflow liés à la synchro data et réduire la pression sur les runners partagés.
